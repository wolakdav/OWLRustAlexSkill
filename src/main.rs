...
fn main() -> Result<(), Box<std::error::Error>> {
    ...
    match userChoice {
        ...
        2 => {let teamInfo = getTeamInfo().unwrap();
            println!("{}",teamInfo);}
        3 => {let rankings = getAllRankings().unwrap();
            println!("{}",rankings);
            }
        _ => println!("oh god oh fuck"),
    };
...
fn getTeamInfo() -> Result<(String),Box<std::error::Error>> {
    ...
    let teamIDs = [7698,4402,7692,4523,4407,7699,7693,4525,4410,4406,4405,4403,7694,4524,4404,4409,4408,7695,7696,7697];
...
println!("
    ...
    if input < 0 || input > 20 
    {
        ...
        apiURL = format!("https://api.overwatchleague.com/teams/{}",teamIDs[(input-1) as usize].to_string());//This always give back the same data so this function is bugged =/, looks like ranking call has the correct stats thou so that might be worth a look
  ...
  let teamInfo = json::parse(&(reqwest::get(&apiURL[..])?.text()?).to_string(), ).unwrap();
    ...
    let result = format!("{} is currently {} place in the league.
    During the current/last stage they had {} match wins,{} match losses, and {} match ties.
    During that time they had won {} maps, lost {} maps, and tied on {} maps",teamName,teamRank,matchWins,matchLoss,matchDraw,gameWin,gameLoss,gameTie);
            ...
            if (matches[i]["state"] == "PENDING") {
                ...
                if (startDate != "null") && (firstContendor != "null") & (secondContendor != "null")
                {
                    ...
                    return format!(
                        ...
                        firstContendor, firstContendorRanking,secondContendor,secondContendorRanking, startDate
                    );
                } else {
